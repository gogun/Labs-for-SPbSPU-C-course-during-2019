...
int main()
{
  ...
  moreva::Rectangle rectangle1({2, 2}, 20, 30);
  circle1.printInfo();
  rectangle1.printInfo();

  moreva::Shape * shape = &circle1;

  //Демонстрация перемещения фигуры
  //1) смещение в заданную точку
  shape->move({12, 12});
  ...
  const moreva::rectangle_t boundedRectRectangle = shape->getFrameRect();
