...
BOOST_AUTO_TEST_CASE(unalteredRectangleAfterMove1)
{
  ...
  BOOST_CHECK_CLOSE_FRACTION(area_before, rect.getArea(), inaccuracy);
  BOOST_CHECK_CLOSE_FRACTION(rect_before.height, rect.getFrameRect().height, inaccuracy);
  BOOST_CHECK_CLOSE_FRACTION(rect_before.width, rect.getFrameRect().width, inaccuracy);
}
...
BOOST_AUTO_TEST_CASE(unalteredCircleAfterMove1)
{
  ...
  circle.move({5.0, 7.0});
  BOOST_CHECK_CLOSE_FRACTION(area_before, circle.getArea(), inaccuracy);
  BOOST_CHECK_CLOSE_FRACTION(rect_before.height, circle.getFrameRect().height, inaccuracy);
  BOOST_CHECK_CLOSE_FRACTION(rect_before.width, circle.getFrameRect().width, inaccuracy);
}
...
BOOST_AUTO_TEST_CASE(quadraticChangeRectangleArea)
{
  ...
  BOOST_CHECK_CLOSE_FRACTION(area_before * coefficient * coefficient, rect.getArea(), inaccuracy);
}
...
BOOST_AUTO_TEST_CASE(quadraticChangeCircleArea)
{
  ...
  BOOST_CHECK_CLOSE_FRACTION(area_before * coefficient * coefficient, circle.getArea(), inaccuracy);
}
