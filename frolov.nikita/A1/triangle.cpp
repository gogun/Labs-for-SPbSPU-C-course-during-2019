...
{
  const point_t vector = {(point2.x - point1.x), (point2.y - point1.y)};
  return sqrt(vector.x * vector.x + vector.y * vector.y);
}

double Triangle::getArea() const
{
  const double lengthVector1 = findLengthVector(point0_, point1_);
  const double lengthVector2 = findLengthVector(point0_, point2_);
  const double lengthVector3 = findLengthVector(point1_, point2_);
